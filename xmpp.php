...
class rXmpp
{
	...
	public function set()
	{
		// PHP 8 fix: $HTTP_RAW_POST_DATA was removed; read directly from php://input
		$HTTP_RAW_POST_DATA = file_get_contents( "php://input" );

		if( $HTTP_RAW_POST_DATA !== false && $HTTP_RAW_POST_DATA !== "" )
		{
			...
			// PHP 8 fix: initialise locals so they are never undefined
			$jabberHost = "";
			$jabberPort = 0;
			$useEncryption = 1;
			...
			foreach( $vars as $var )
			{
				...
				{
					$useEncryption = (int)$parts[1];
				}
				else if( $parts[0] == "advancedSettings" )
				{
					$this->advancedSettings = (int)$parts[1];
				}
